...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($show_price && (empty($wholesale_price) || !is_numeric($wholesale_price) || $wholesale_price <= 0)) {
        $errors['wholesale_price'] = 'O preço deve ser um valor numérico positivo quando o produto tem preço';
    }
    ...
    if ($has_min_quantity && (empty($min_wholesale_quantity) || $min_wholesale_quantity <= 0)) {
        $errors['min_wholesale_quantity'] = 'A quantidade mínima deve ser maior que zero quando habilitada';
    }
    ...
    if (empty($errors)) {
        ...
        $query = "UPDATE products 
                 ...
                 min_wholesale_quantity = ?, unit_weight = ?, unit_type = ?, unit_display_name = ?, 
                 stock = ?, image_url = ?, category_id = ?, featured = ?, promotion = ?, status = ?, 
                 show_price = ?, has_min_quantity = ?, updated_at = NOW() 
                 WHERE id = ?";
        
        $stmt = $conn->prepare($query);
        $stmt->bind_param("ssddisssiiiiiiiii", 
                         $name,                      // s - string
                         $description,               // s - string
                         $wholesale_price_db,        // d - double (nullable)
                         $retail_price_db,           // d - double (nullable)
                         $min_wholesale_quantity_db, // i - int (nullable)
                         $unit_weight,               // s - string (pode ser decimal)
                         $unit_type,                 // s - string
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-check">
                                    <input type="checkbox" class="form-check-input" id="show_price" name="show_price" <?php echo $show_price ? 'checked' : ''; ?> onchange="togglePriceFields()">
                            ...
                            <div class="col-md-6">
                                <div class="form-check">
                                    <input type="checkbox" class="form-check-input" id="has_min_quantity" name="has_min_quantity" <?php echo $has_min_quantity ? 'checked' : ''; ?> onchange="toggleMinQuantityField()">
                    ...
                    <div class="row mb-3" id="price_section">
                        <div class="col-md-6">
                            ...
                            <div class="input-group has-validation">
                                ...
                                <input type="text" class="form-control <?php echo isset($errors['wholesale_price']) ? 'is-invalid' : ''; ?>" id="wholesale_price" name="wholesale_price" value="<?php echo $wholesale_price ? number_format($wholesale_price, 2, ',', '.') : ''; ?>" required>
                                <div class="invalid-feedback">
...
<script>
    ...
    function updateUnitLabels() {
        ...
        if (unitType === 'kg') {
            ...
            if (unitDisplayName.value === 'unidades' || unitDisplayName.value === '') {
            ...
            if (unitDisplayName.value === 'kg' || unitDisplayName.value === '') {
